...
class Role(SurrogatePK, Model):
    ...
    __tablename__ = 'info_roles'
    name = Column(db.String(255), unique=True, nullable=False)
    user_id = reference_col('info_users', nullable=True)
    user = relationship('User', backref='info_roles')
...
class User(UserMixin, SurrogatePK, Model):
    ...
    #: The hashed password
    password = Column(db.Binary(255), nullable=True)
